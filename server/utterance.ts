...
export const CONTINUATION_ENDPOINT_GRACE_MS = 1300;
...
function isAmbiguousTurn(text: string): boolean {
  ...
  const incompleteEnding = /\b(and|or|but|because|so|if|when|where|from|to|with|of|for|about|the|a|an|your|my|our|their|how)$/;
  const incompleteLead = /\b(tell me(?: about)?|can you explain|what about|how about|where do|where are|what is|what are|how do|how did|and how|for example|in your current role|can you|could you|would you)\??$/;
  const hasTerminalPunctuation = /[.!?]$/.test(normalized);

  return greeting.test(phrase) || !hasTerminalPunctuation || (!normalized.endsWith("?") && words.length < 3) || incompleteEnding.test(phrase) || incompleteLead.test(phrase);
}

function getEndpointGraceDelay(text: string): number {
  if (isAmbiguousTurn(text)) return AMBIGUOUS_SILENCE_MS;
  return text.endsWith("?") ? CLEAR_ENDPOINT_GRACE_MS : CONTINUATION_ENDPOINT_GRACE_MS;
}
...
export class InterviewerUtteranceBuffer {
  ...
  completeFromUtteranceEnd() {
    ...
    this.schedule("Deepgram UtteranceEnd", getEndpointGraceDelay(this.utterance));
  }
